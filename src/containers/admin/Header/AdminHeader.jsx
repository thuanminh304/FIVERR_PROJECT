...
const Adminheader = () => {
  ...
  const { isFixSideBar, themeColor } = useSelector(
    (state) => state.AdminDashBoardSettingReducer
  );
  const { isNote, listNote, isNewNotify } = useSelector(
    (state) => state.AdminDashBoardSettingReducer
  );
      ...
      <div className="adminHeader-container">
        <div className="adminHeader-content">
          <div className="header__left">
            <div
              className={"header__logoBranch " + (!isFixSideBar ? "inFix" : "")}
            >
              <h4>{!isFixSideBar ? "F" : "FiverrFake"}</h4>
            </div>
            <div
              className={
                "header__humbergetBtn " + (!isFixSideBar ? "inFix" : "")
              }
              onClick={fixSideBar}
            >
              <ul>
        ...
        <div
          className={"adminHeader-sideBar " + (showSideRight ? "show" : "")}
        ></div>
