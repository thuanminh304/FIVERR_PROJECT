...
const Listsubjob = (props) => {
  ...
  const { jobId } = props;
  const { mainJob, loadingAction, isError } = useSelector(
  ...
  const editFunc = (e) => {
    ...
    const subJobItemSelected =
      subJobTypeBox[key].querySelector(".subJobType__title");
    const subJobItemEditing = subJobTypeBox[key].querySelector(
      ".subJobType__settingContent"
    );
  ...
  const approveSubJob = (e) => {
    const subJobTypeBox = document.querySelectorAll(".subJobType-item");
    const subJobItemSelected =
      subJobTypeBox[editState.key].querySelector(".subJobType__title");
    const subJobItemEditing = subJobTypeBox[editState.key].querySelector(
      ".subJobType__settingContent"
    );
    ...
    <div className="subTypeJob-content">
      ...
      <div className="JobType-list">
        <div className="subJobType-content row">
          {mainJob[jobId]?.subTypeJobs.map((subJobType, idx) => {
            return (
              <div
                ...
                className={
                  "subJobType-item col-12 col-sm-4 col-lg-3 " +
              ...
              >
                <div className="subJobType__content row">
                  ...
                  <div className="subJobType__title">{subJobType.name}</div>
                </div>
              </div>
            );
          })}
          <div
            className={
              "subJobType-item col-md-3 " + (!editState ? "" : "disabled")
            }
          >
            <div
              className={
                "subJobType__content subJobType-add row " +
                (isAdd ? "addnew" : "")
              }
              onClick={addNewSubJob}
            >
              <div className="subJobType__settingContent">
                {renderAddSubJob()}
              </div>
                    ...
                    rules={[
                      ...
                      ({ getFieldValue }) => ({
                        validator(_, value) {
                          let isDublicate = -1;
                          if (value) {
                            isDublicate = mainJob[jobId]?.subTypeJobs.findIndex(
                              (job) => {
                                return (
